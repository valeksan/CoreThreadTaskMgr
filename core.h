...
{
    template<int N>
    struct is_placeholder<my_placeholder<N> > : public integral_constant<int, N> {};
}

template<typename R, typename...Args, typename Class, std::size_t ...N>
auto bind_placeholders(R(Class::*taskFunction)(Args...), Class *taskObj, std::index_sequence<N...>) { return std::bind(taskFunction, taskObj, my_placeholder<N+1>()...); }
...
public:
    ...
    void addTask(int taskType, Args&&... args)
    {
        ...
        try
        {
            ...
            auto argsList = QVariantList({ QVariant::fromValue(args)... });
            auto taskFunctionBound = std::bind(taskFunction, std::forward<Args>(args)...);
